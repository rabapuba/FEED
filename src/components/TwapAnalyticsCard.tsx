...
export const TwapAnalyticsCard: React.FC<TwapAnalyticsCardProps> = ({
  ...
  return (
    ...
    >
      {/* Title */}
      <div className={`flex items-center justify-between pb-1.5 border-b mb-2 ${isDark ? 'border-[#2b313a]' : 'border-slate-200'}`}>
        <div className={`flex items-center space-x-1.5 font-bold text-[11px] uppercase ${isDark ? 'text-[#eaecef]' : 'text-slate-800'}`}>
          <ShieldCheck className="w-3.5 h-3.5 text-[#9353d3]" />
          <span>CHAINLINK TWAP BENCHMARK</span>
        </div>
        <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#9353d3]/20 border border-[#9353d3]/40 text-[#b57bee] font-black">
          STREAM 60S
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        {/* Strike Price */}
        <div className={`p-1.5 rounded-lg border ${isDark ? 'bg-[#1e2329] border-[#2b313a]' : 'bg-slate-50 border-slate-200'}`}>
          <span className="text-[9px] text-[#848e9c] font-bold uppercase block">STRIKE (00:00)</span>
          ...
          <span className="text-[8px] text-[#848e9c] block">Benchmark Ronde</span>
        </div>

        {/* Running TWAP */}
        <div className={`p-1.5 rounded-lg border ${isDark ? 'bg-[#1e2329] border-[#2b313a]' : 'bg-slate-50 border-slate-200'}`}>
          <span className="text-[9px] text-[#848e9c] font-bold uppercase block">RUNNING TWAP</span>
          <span className="text-sm font-black text-[#b57bee]">
            ${runningTwap > 0 ? runningTwap.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '---.--'}
          </span>
          <span className={`text-[9px] font-black ${isUpWinning ? 'text-[#0ecb81]' : 'text-[#f6465d]'}`}>
      ...
      <div className={`p-2 rounded-lg border ${isDark ? 'bg-[#1e2329] border-[#2b313a]' : 'bg-slate-50 border-slate-200'}`}>
        <div className="flex items-center justify-between mb-0.5">
          <div className="flex items-center space-x-1 text-[#848e9c] font-bold text-[10px]">
            <Target className="w-3 h-3 text-[#f0b90b]" />
            <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>TARGET HARGA BALIK:</span>
          </div>
          <span className="text-[9px] text-[#848e9c] font-medium">Sisa {secondsLeft}s</span>
        </div>
      ...
      {/* Market Link */}
      <div className={`flex items-center justify-between text-[10px] pt-1.5 border-t text-[#848e9c] mt-2 ${isDark ? 'border-[#2b313a]' : 'border-slate-200'}`}>
